...
% 定义中英文摘要和关键字
\begin{cabstract}
Web API在互联网软件中正被广泛应用. Web API可作为公开服务提供, 并集成于应用中, 满足各种用户需求. 因此, 作为应用的基础构件, web API的测试十分必要. 然而, 由于需要大量测试用例, 手动测试具有很高代价.

本文提出了一种基于场景模型的web API自动化测试方法: 定义了有效描述API数据流与状态流约束的场景模型; 提出了基于场景模型自动化生成测试用例的方法. 本文实现了原型工具, 并在实际web API服务上对工具进行了评估, 验证了模型和方法的有效性.
